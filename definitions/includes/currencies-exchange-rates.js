...
function requireTemplateSettings () {
	...
	var escape = require_escape(),
	    reEscape = require_reEscape(),
	    reEvaluate = require_reEvaluate(),
	    reInterpolate = require_reInterpolate();
...
function requireTemplate () {
	...
	var assignInWith = requireAssignInWith(),
	    attempt = requireAttempt(),
	    baseValues = require_baseValues(),
	    customDefaultsAssignIn = require_customDefaultsAssignIn(),
	    escapeStringChar = require_escapeStringChar(),
	    isError = requireIsError(),
	    isIterateeCall = require_isIterateeCall(),
	    keys = requireKeys(),
	    reInterpolate = require_reInterpolate(),
	    templateSettings = requireTemplateSettings(),
	    toString = requireToString();
...
function requireNode_cache () {
	...
	(function() {
	  // lodash requires
	  var EventEmitter, _assignIn, _isArray, _isFunction, _isNumber, _isObject, _isString, _size, _template, clone,
	    boundMethodCheck = function(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new Error('Bound instance method accessed before binding'); } },
	    indexOf = [].indexOf;

	  _assignIn = requireAssignIn();

	  _isArray = requireIsArray();

	  _isString = requireIsString();

	  _isFunction = requireIsFunction();

	  _isNumber = requireIsNumber();

	  _isObject = requireIsObject();

	  _size = requireSize();

	  _template = requireTemplate();

	  clone = requireClone();

	  EventEmitter = require$$9.EventEmitter;
...
function requireNodeCache () {
	...
	(function() {
	  ...
	  exports = nodeCache$1.exports = requireNode_cache();
...
const LATEST = 'latest';
const exchangeApi = 'https://api.exchangeratesapi.io/';
...
const convertCurrency = async (from, to, amount, date) =>  await CurrencyService.convert(from, to, amount, date);
